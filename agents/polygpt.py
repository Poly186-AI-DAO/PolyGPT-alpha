...
from utils.autogen.autogen_function_reaction import PolyAutogenReact
from utils.autogen.autogen_monitor import AutogenMonitor
from utils.poly_observer import PolyObservable
from utils.poly_logger import PolyLogger, LogLevel  # Import LogLevel if needed

# Initialize logger
logger = PolyLogger(__name__)

class PolyGPTAgents(PolyObservable):
    def __init__(self):
        super().__init__()
        self.agent_monitors = []
    ...
    def _initialize_agents(self):
        ...
        for agent_name, agent in self.agents.items():
            if agent_name not in self.reactions:
                self.reactions[agent_name] = PolyAutogenReact(agent)

            agent_monitor = AutogenMonitor(agent, self, self.groupchat, self.manager)
    ...
    async def receive_notification(self, event: str, data: Any):
        try:
            ...
            if agent_reactions:
                ...
                reaction_method = getattr(agent_reactions, reaction_method_name, agent_reactions.default_reaction)
            ...
            logger.error(f"Error in receive_notification: {e}")  # Use logger.error instead of LOG.error
